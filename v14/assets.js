...
const CLUB_ALIASES={
  ...
  'Arsenal':['Arsenal'], 'Chelsea':['Chelsea'], 'Liverpool':['Liverpool'], 'Manchester City':['Manchester City'], 'Manchester United':['Manchester United'],
  'Newcastle':['Newcastle United','Newcastle'], 'Tottenham':['Tottenham Hotspur','Tottenham'], 'Aston Villa':['Aston Villa'], 'Everton':['Everton'],
  'PSG':['Paris Saint-Germain','Paris SG','PSG'], 'Bayern München':['Bayern Munich','FC Bayern Munich','Bayern München'], 'Inter':['Inter Milan','Internazionale','Inter'],
  'AC Milan':['AC Milan','Milan'], 'Real Sociedad':['Real Sociedad'], 'Deportivo de La Coruña':['Deportivo La Coruna','Deportivo de La Coruña'],
...
// Rectangular internet photos are intentionally discarded, including all old Sabadell portraits.
if(PV.state.assetPolicyVersion!==14){
  PV.state.photos={};
  PV.state.photoSourcesV12={};
  PV.state.photoMetaV14={};
  PV.state.photoMissesV14={};
  PV.state.assetPolicyVersion=14;
  PV.save?.();
}
PV.state.photoMetaV14=PV.state.photoMetaV14||{};
PV.state.photoMissesV14=PV.state.photoMissesV14||{};
  ...
  const job=(async()=>{
    ...
    for(const q of namesFor(p)){
      try{
        ...
        if(!r.ok){transient=true;continue}
        completed++;
        ...
        if(url){
          ...
          PV.state.photoMetaV14[p.id]={club:p.club,kind:'cutout',source:'TheSportsDB',verifiedAt:new Date().toISOString().slice(0,10)};
          delete PV.state.photoMissesV14[p.id];PV.save?.();return url;
        }
...
PV.hydrate=root=>{
  ...
  [...scope.querySelectorAll('[data-photo]')].forEach(n=>{const id=n.dataset.photo,p=PV.byId(id);if(!p)return;const meta=PV.state.photoMetaV14[id],u=PV.state.photos[id];if(u&&meta?.club===p.club&&meta?.kind==='cutout')paint(id,u);else{n.querySelector('img')?.remove();if(obs)obs.observe(n);else enqueue(id)}});
};
PV.assetCoverage=()=>{
...
window.PV14_ASSET_AUDIT={policy:'cutout-only-current-club',rectangularPhotosAllowed:false,silhouettePreferred:true,coverage:PV.assetCoverage};
})();